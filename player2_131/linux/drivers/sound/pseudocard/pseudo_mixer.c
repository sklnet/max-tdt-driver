...
static const struct snd_pseudo_mixer_downstream_topology default_topology[] = {
   {
      {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,17)
   #if defined(__TDT__) && ! defined(UFS922) && ! defined(UFC960)
...
#elif defined CONFIG_CPU_SUBTYPE_STX7141 || defined CONFIG_CPU_SUBTYPE_STX7105 || defined CONFIG_CPU_SUBTYPE_STX7111 
...
struct snd_pseudo {
	...
	struct snd_kcontrol *dynamic_controls[16];
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	spin_lock_irq(&pseudo->mixer_lock);
	...
	spin_unlock_irq(&pseudo->mixer_lock);
	return 0;
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (changed) {
		...
		if (addr == PSEUDO_ADDR(downstream_topology))
			/* we own locks at this point that must be dropped before
			 * we can fully update the topology.
			 */ 
			schedule_work(&pseudo->work);
	}
...
static void __init snd_pseudo_mixer_init(struct snd_pseudo *pseudo, int dev)
{
	...
	memset(mixer, 0, sizeof(*mixer));
        ...
        /* emergency mute */
        mixer->emergency_mute = SND_PSEUDO_MIXER_EMERGENCY_MUTE_SOURCE_ONLY;
        
        /* switches */
        mixer->dc_remove_enable = 1; /* On */
...
static void snd_card_pseudo_update_dynamic_controls(struct work_struct *work)
{
	struct snd_pseudo *pseudo = container_of(work, struct snd_pseudo, work);
	int err;
		...
		struct snd_pseudo_mixer_downmix_index *index)
{
	...
	for (i=0; i<header->num_index_entries; i++) {
		...
		uint64_t sort_value = ((uint64_t) output_id.n << 32) + input_id.n;
