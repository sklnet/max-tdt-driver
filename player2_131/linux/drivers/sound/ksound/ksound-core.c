 ...
 * (BWC vs. BWD)
 */
#if defined(__TDT__) && (defined(FORTIS_HDBOX) || defined(UFS922) || defined(UFC960) || defined(HL101) || defined(VIP1_V2) || defined(VIP2_V1) || defined(OCTAGON1008))
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30)
...
static void _ksnd_pcm_release_substream(snd_pcm_substream_t *substream)
{
...
static int _ksnd_pcm_open_substream(snd_pcm_t *pcm,
				    int stream,
				    snd_pcm_substream_t **r_substream,
				    struct file *file)
{
	int err = 0;
	snd_pcm_substream_t *psubstream;

	if ((err =
	     snd_pcm_open_substream(pcm, stream, file,
				    &psubstream)) < 0)
		return err;
	...
	if (err < 0) {
		_ksnd_pcm_release_substream(psubstream);
		return err;
	}
		  ...
		  snd_pcm_stream_t stream)
{
	...
	err = _ksnd_pcm_open_substream(pcm, stream_type, &(xkpcm->substream),
				       &default_file);
...
void ksnd_pcm_close(ksnd_pcm_t *kpcm)
{
	...
	mutex_lock(&pcm->open_mutex);
	_ksnd_pcm_release_substream(substream);
	mutex_unlock(&pcm->open_mutex);
