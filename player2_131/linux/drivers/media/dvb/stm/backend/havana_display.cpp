...
#if defined(UFS922) || defined(UFC960) || defined(TF7700)  || defined(CUBEREVO) || defined(CUBEREVO_MINI2) || defined(CUBEREVO_MINI) || defined(CUBEREVO_250HD) || defined(CUBEREVO_2000HD) || defined(CUBEREVO_9500HD) || defined(CUBEREVO_MINI_FTA)
//Dagobert: lmi_vid isn't available for pip :(
        ...
        if (SurfaceId == DISPLAY_ID_PIP)
        {
            ...
            ManifestorStatus        = VideoManifestor->GetSurfaceParameters ((void**)&SurfaceDescriptor);
            if (ManifestorStatus != ManifestorNoError)
            {
                HAVANA_ERROR("Failed to access manifestor surface\n");
                return HavanaError;
            }
            ManifestorStatus        = VideoManifestor->SetOutputWindow (((SurfaceDescriptor->DisplayWidth*PIP_X_N)/PIP_FACTOR_D)-PIP_X_OFFSET,
                                                                        PIP_Y_OFFSET,
                                                                        (SurfaceDescriptor->DisplayWidth*PIP_FACTOR_N)/PIP_FACTOR_D,
                                                                        (SurfaceDescriptor->DisplayHeight*PIP_FACTOR_N)/PIP_FACTOR_D);
            if (ManifestorStatus != ManifestorNoError)
            {
                HAVANA_ERROR("Failed to set output window dimensions\n");
                return HavanaError;
            }
        }
        #endif
